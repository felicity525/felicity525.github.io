...
- 当需要修改某一区间时，我们只标记这个区间，并不真正修改它。
- 等到需要查询这个区间时，我们遍历这个区间，并将标记过的节点的值更新到树上。

#### 具体步骤

为了实现懒惰标记，线段树的每个节点除了存储区间信息（例如 `sum`），还需要一个额外的变量 `lazy`。

1. **构建 (Build)**：
    - 懒惰标记通常在构建时初始化为 0 或其他空值。
2. **懒惰标记下传 (`push_down`)**：
    - 负责将父节点的懒惰标记传递给子节点。
    - 检查当前节点 `node` 的 `lazy` 标记是否非零。
    - 如果非零，说明有待处理的更新：
        - 将 `lazy` 标记代表的更新值累加到左子节点和右子节点的 `lazy` 标记上。
        - 根据更新值，更新左子节点和右子节点的**值**（例如 `sum`）。
        - 清除当前节点的 `lazy` 标记。
3. **区间更新 (Range Update)**：
    - 递归地从根节点开始，更新目标区间 $[U_L,U_R]$。
    - **三种情况**：
        - **当前节点区间与更新区间完全重合**：更新当前节点的值，并设置其 `lazy` 标记。然后直接返回，不继续递归。
        - **当前节点区间与更新区间部分重叠**：
            1. **下传**当前节点的懒惰标记，确保子节点的值是最新的。
            2. **合并**左右子节点的值，更新当前节点的值。
        - **当前节点区间与更新区间不重叠**：直接返回。
4. **区间查询 (Range Query)**：
    - 递归地从根节点开始，查询目标区间$[Q_L,Q_R]$。
    - **两种情况**：
        - **当前节点区间与查询区间完全重合**：直接返回当前节点的值。
        - **当前节点区间与查询区间部分重叠**：
            1. **下传**当前节点的懒惰标记。
            2. 递归地向左、右子节点进行查询。
            3. 合并左右子节点的结果并返回。
...
void push_down(int node, int start, int end) {
    if (lazy[node]) {
        ...
        tree[node * 2] += (mid-start+1)lazy[node];
        tree[node * 2 + 1] += (end-mid)lazy[node];//更新左右结点的值

        lazy[node * 2] += lazy[node];
        lazy[node * 2 + 1] += lazy[node];//下传懒惰标记

        lazy[node] = 0;//清空懒惰标记
    }
}
void update(int node, int start, int end, int l, int r, int val) {//区间更新
    if(r<start||end<l)
        return ;
    if(l<=start&&end<=r)//完全重合
    {
